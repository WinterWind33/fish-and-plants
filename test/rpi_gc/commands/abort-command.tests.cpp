...
TEST_CASE("AbortCommand execution unit tests",
          "[unit][solitary][rpi_gc][commands][AbortCommand][execution]") {
    using rpi_gc::commands::AbortCommand;
    using testing::NiceMock;
    using testing::StrictMock;

    GIVEN("An abort command with one emergency stoppable system") {
        auto stoppableSystemMock = std::make_shared<
            StrictMock<rpi_gc::abort_system::mocks::EmergencyStoppableSystemMock>>();
        ...
        AbortCommand commandUnderTest{
            loggerMock, {stoppableSystemMock}, std::move(optionParserMock)};

        WHEN("execute() is called") {
            THEN("emergencyAbort() should be called on the stoppable system") {
                ON_CALL(optionParserMockRef, getOptions())
                    .WillByDefault(testing::Return(
                        std::vector<AbortCommand::option_parser::option_pointer>{}));
