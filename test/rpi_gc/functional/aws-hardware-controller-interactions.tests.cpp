...
namespace tests {
    ...
    static std::pair<std::unique_ptr<digital_pin_mock>, digital_pin_mock*> createDigitalPinMock() noexcept {
...
SCENARIO("User changes the board PINs during a Daily Cycle", "[functional][non-regression][automatic-watering][hardware-management][user-interaction]") {
    ...
    EXPECT_CALL(boardChipMock,
        requestDigitalPin(testing::_, tests::constants::VALVE_DUMMY_ID, gh_hal::hardware_access::DigitalPinRequestDirection::Output))
        .WillOnce(testing::Return(testing::ByMove(std::move(valveMockUniquePtr))));

    EXPECT_CALL(boardChipMock,
        requestDigitalPin(testing::_, tests::constants::PUMP_DUMMY_ID, gh_hal::hardware_access::DigitalPinRequestDirection::Output))
    ...
    rpi_gc::automatic_watering::DailyCycleAWSHardwareController awsController{
        ...
        std::ref(boardChipMock),
        tests::constants::VALVE_DUMMY_ID,
        tests::constants::PUMP_DUMMY_ID
    };
    std::atomic<rpi_gc::automatic_watering::WateringSystemHardwareController*> hardwareControllerAtomic{&awsController};
    ...
    rpi_gc::automatic_watering::DailyCycleAutomaticWateringSystem::time_provider_pointer awsTimeProvider{&timeProvider};
    ...
    WHEN("When the automatic watering system is started") {
        ...
        testing::Expectation pumpActivationExp = EXPECT_CALL(*pumpMockPtr, activate)
            .After(valveActivationExp);
        testing::Expectation valveDeactivationExp = EXPECT_CALL(*valveMockPtr, deactivate)
            .After(pumpActivationExp);
        testing::Expectation pumpDeactivationExp = EXPECT_CALL(*pumpMockPtr, deactivate)
            .After(valveDeactivationExp)
            .RetiresOnSaturation();
