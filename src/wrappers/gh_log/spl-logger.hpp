...
public:
    ...
    [[nodiscard]] static std::shared_ptr<SPLLogger> createFileLogger(
        const std::string& name, const std::filesystem::path& filepath) noexcept;

    [[nodiscard]] static std::shared_ptr<SPLLogger> createColoredStdOutLogger(
        const std::string& name) noexcept;

    [[nodiscard]] static std::shared_ptr<SPLLogger> createDailyRotatingLogger(
        const std::string& name) noexcept;
