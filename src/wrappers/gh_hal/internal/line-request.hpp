...
public:
    ...
    [[nodiscard]] constexpr to_type convert(const from_type fromValue) const {
...
#ifdef USE_LIBGPIOD
using DirectionEnumConverter =
    EnumConverter<hardware_access::DigitalPinRequestDirection, ::gpiod::line::direction, 2>;

constexpr DirectionEnumConverter LibgpiodConverter{
    DirectionEnumConverter::map_type{
                                     DirectionEnumConverter::entry_type{hardware_access::DigitalPinRequestDirection::Input,
                                           ::gpiod::line::direction::INPUT},
                                     DirectionEnumConverter::entry_type{hardware_access::DigitalPinRequestDirection::Output,
                                           ::gpiod::line::direction::OUTPUT}}
};

static_assert(LibgpiodConverter.convert(hardware_access::DigitalPinRequestDirection::Input) ==
              ::gpiod::line::direction::INPUT);
static_assert(LibgpiodConverter.convert(hardware_access::DigitalPinRequestDirection::Output) ==
              ::gpiod::line::direction::OUTPUT);
#endif // USE_LIBGPIOD

using FakeLineRequest = std::pair<hardware_access::DigitalPinRequestDirection,
                                  std::vector<backends::simulated::DigitalBoardPin>>;
...
public:
#ifdef USE_LIBGPIOD
    using backend_type = std::pair<hardware_access::DigitalPinRequestDirection,
                                   backends::libgpiod_impl::NativeLineRequestType>;
    ...
    explicit LineRequest(const consumer_type& consumer, chip_reference chip,
    ...
    [[nodiscard]] std::vector<std::unique_ptr<hardware_access::BoardDigitalPin>> getBoardPins()
        const noexcept;
