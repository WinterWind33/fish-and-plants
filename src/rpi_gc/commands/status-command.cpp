...
StatusCommand::StatusCommand(option_parser_ptr optionParser,
...
void StatusCommand::printHelp(help_ostream_type outputStream) const noexcept {
    ...
    outputStream.get() << "\t" << strings::commands::STATUS
                       << " - display the status of all the active systems" << std::endl;
    ...
    outputStream.get()
        << "\t"
        << "Retrieves status from all the active systems in the applications (AWS flows etc)"
        << std::endl;
    outputStream.get()
        << "\tand it displays them in a structured way, every system separated by a section."
        << std::endl;
