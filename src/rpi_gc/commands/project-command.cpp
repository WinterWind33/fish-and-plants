...
ProjectCommand::ProjectCommand(option_parser_pointer optionParser,
...
bool ProjectCommand::execute() noexcept {
    ...
    auto optionIt =
        std::find_if(std::begin(options), std::end(options),
                     [](const option_parser::option_pointer& optionPointer) {
...
void ProjectCommand::printHelp(help_ostream_type outputStream) const noexcept {
    ...
    outputStream.get() << "\t" << strings::commands::PROJECT
                       << " - manages the project of an automated greenhouse" << std::endl;
    ...
    outputStream.get()
        << "\t"
        << "Manages the project information and configuration of an automated greenhouse. It is"
        << std::endl;
    outputStream.get() << "\tcapable of handling the creation, loading and saving of the projects."
                       << std::endl;
    outputStream.get()
        << "\tWARNING: Due to limitations of the underlying parsing engine, please DO NOT use ";
