...
public:
    ...
    explicit HelpCommand(ostream_ref outputStream,
    ...
    [[nodiscard]] constexpr name_type getName() const noexcept override {
        return strings::commands::HELP;
    }

    constexpr bool processInputOptions(const std::vector<string_type>& options) override {
        return true;
    }
    ...
    [[nodiscard]] option_pointer getAsOption() const noexcept override {
        return m_asOption;
    }
...
private:
    ...
    [[nodiscard]] static std::string get_app_version() noexcept;
